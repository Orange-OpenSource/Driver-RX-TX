...
final public class RXTXPort extends SerialPort
{
	...
	*  @see gnu.io.SerialPort
	*/
	public RXTXPort( String name ) throws PortInUseException, NoSuchPortException
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:RXTXPort("+name+") called");
		fd = open( name );
		if(fd < 0){
			if(fd == -2)
				throw new NoSuchPortException("rxtxport native open failed, no such device file");
			if(fd == -1)
				throw new PortInUseException("rxtxport native open failed, port used by another application");
		}
		this.name = name;

		MonitorThreadLock = true;
		monThread = new MonitorThread();
		monThread.setName("RxTxPort_" + name + "_Thread");
		monThread.start();
		waitForTheNativeCodeSilly();
		MonitorThreadAlive=true;
		timeout = -1;	/* default disabled timeout */
		if (debug)
			RXTXlog.printDebug( "RXTXPort:RXTXPort("+name+") returns with fd = " +
				fd);
		
	}
	
//	public static RXTXPort getRXTXPort(String name){
//		if(open(name) < 0)
//			return null;
//		else 
//			return new RXTXPort(name);
//	}
	
	
	private native synchronized int open( String name )
		throws PortInUseException, NoSuchPortException;
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:getOutputStream() called and returning");
		return out;
	}
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:getInputStream() called and returning");
		return in;
	}
...
//	private native int nativeGetFlowControlMode( int fd );
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:setSerialPortParams(" +
				b + " " + d + " " + s + " " + p + ") called");
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:getBaudRate() called and returning " + speed);
		return speed;
	}
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:getDataBits() called and returning " + dataBits);
		return dataBits;
	}
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:getStopBits() called and returning " + stopBits);
		return stopBits;
	}
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:getParity() called and returning " + parity );
		return parity;
	}
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:setFlowControlMode( " + flowcontrol + " ) called");
		if(monThreadisInterrupted) 
		{
			if( debug_events )
				RXTXlog.printDebug(  "RXTXPort:setFlowControlMode MonThread is Interrupeted returning" );
			return;
		}
		try {
		...
		if (debug)
			RXTXlog.printDebug( "RXTXPort:setFlowControlMode( " + flowcontrol + " ) returning");
	}
	/** 
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:getFlowControlMode() returning " + flowmode );
		return flowmode;
	}
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:enableReceiveFramming() throwing exception");
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:disableReceiveFramming() called and returning (noop)");
	}
	/** 
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:isReceiveFrammingEnabled() called and returning " + false );
		return false;
	}
	/** 
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:getReceiveFrammingByte() called and returning " + 0 );
		return 0;
	}
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:disableReceiveTimeout() called");
		timeout = -1;
		NativeEnableReceiveTimeoutThreshold( timeout , threshold, InputBuffer );
		if (debug)
			RXTXlog.printDebug( "RXTXPort:disableReceiveTimeout() returning");
	}
	/** 
	*  @param time
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:enableReceiveTimeout() called");
		if( time >= 0 )
		{
			timeout = time;
		...
		if (debug)
			RXTXlog.printDebug( "RXTXPort:enableReceiveTimeout() returning");
	}
	/** 
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:isReceiveTimeoutEnabled() called and returning " + NativeisReceiveTimeoutEnabled() );
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:getReceiveTimeout() called and returning " + NativegetReceiveTimeout() );
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:enableReceiveThreshold( " + thresh + " ) called");
		if(thresh >=0)
		{
			threshold=thresh;
		...
		if (debug)
			RXTXlog.printDebug( "RXTXPort:enableReceiveThreshold( " + thresh + " ) returned");
	}
	/** 
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:disableReceiveThreshold() called and returning");
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:getReceiveThreshold() called and returning " + threshold);
		return threshold;
	}
	/** 
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:isReceiveThresholdEnable() called and returning" + (threshold > 0) );
		return(threshold>0);
	}
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:setInputBufferSize( " +
					size + ") called");
		if( size < 0 )
			throw new IllegalArgumentException
			(
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:getInputBufferSize() called and returning " + InputBuffer );
		return(InputBuffer);
	}
	/** 
	*  @param size
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:setOutputBufferSize( " +
					size + ") called");
		if( size < 0 )
			throw new IllegalArgumentException
			(
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:getOutputBufferSize() called and returning " + OutputBuffer );
		return(OutputBuffer);
	}
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:checkMonitorThread()");
		if(monThread != null)
		{
			if ( debug )
				RXTXlog.printDebug( 
		...
		if ( debug )
			RXTXlog.printDebug(  "monThread is null " );
		return(true);
	}
	...
	*/
	...
	{
		if (debug_events)
			RXTXlog.printDebug( "RXTXPort:sendEvent(");
		...
		switch( event )
		{
			case SerialPortEvent.DATA_AVAILABLE:
				if( debug_events )
					RXTXlog.printDebug( "DATA_AVAILABLE " +
						monThread.Data + ")" );
				break;
			...
			case SerialPortEvent.CTS:
				if( debug_events )
					RXTXlog.printDebug( "CTS " +
						monThread.CTS + ")" );
				break;
			case SerialPortEvent.DSR:
				if( debug_events )
					RXTXlog.printDebug( "DSR " +
						monThread.Output + ")" );
				break;
			case SerialPortEvent.RI:
				if( debug_events )
					RXTXlog.printDebug( "RI " +
						monThread.RI + ")" );
				break;
			case SerialPortEvent.CD:
				if( debug_events )
					RXTXlog.printDebug( "CD " +
						monThread.CD + ")" );
				break;
			case SerialPortEvent.OE:
				if( debug_events )
					RXTXlog.printDebug( "OE " +
						monThread.OE + ")" );
				break;
			case SerialPortEvent.PE:
				if( debug_events )
					RXTXlog.printDebug( "PE " +
						monThread.PE + ")" );
				break;
			case SerialPortEvent.FE:
				if( debug_events )
					RXTXlog.printDebug( "FE " +
						monThread.FE + ")" );
				break;
			case SerialPortEvent.BI:
				if( debug_events )
					RXTXlog.printDebug( "BI " +
						monThread.BI + ")" );
				break;
			default:
				if( debug_events )
					RXTXlog.printDebug( "XXXXXXXXXXXXXX " +
						event + ")" );
				break;
		}
		if( debug_events && debug_verbose )
			RXTXlog.printDebug(  "	checking flags " );
		...
		if( debug_events && debug_verbose )
			RXTXlog.printDebug(  "	getting event" );
		SerialPortEvent e = new SerialPortEvent(this, event, !state,
			state );
		if( debug_events && debug_verbose )
			RXTXlog.printDebug(  "	sending event" );
		if(monThreadisInterrupted) 
		{
			if( debug_events )
				RXTXlog.printDebug(  "	sendEvent return" );
			return(true);
		}
	...
	*/
		...
		SerialPortEventListener lsnr ) throws TooManyListenersException
	{
		...
		*/
		...
		if (debug)
			RXTXlog.printDebug( "RXTXPort:addEventListener()");
		if( SPEventListener != null )
		{
		...
		if( !MonitorThreadAlive )
		{
			MonitorThreadLock = true;
			monThread = new MonitorThread();
			monThread.setName("RxTxListener_" + name + "_Thread");
			monThread.start();
			waitForTheNativeCodeSilly();
			MonitorThreadAlive=true;
		}
		if (debug)
			RXTXlog.printDebug( "RXTXPort:Interrupt=false");
	}
	/**
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:removeEventListener() called");
		waitForTheNativeCodeSilly();
		//if( monThread != null && monThread.isAlive() )
		if( monThreadisInterrupted == true )
		{
			RXTXlog.printDebug( "	RXTXPort:removeEventListener() already interrupted");
			monThread = null;
		...
		{
			if (debug)
				RXTXlog.printDebug( "	RXTXPort:Interrupt=true");
			...
			*/
			if (debug)
				RXTXlog.printDebug( "	RXTXPort:calling interruptEventLoop");
			interruptEventLoop( );
			
			if (debug)
				RXTXlog.printDebug( "	RXTXPort:calling monThread.join()");
			try {
				monThread.join(1000);
			} catch (Exception ex) {
				/* yikes */
			...
			if (debug)
				RXTXlog.printDebug( "	RXTXPort:waiting on isAlive()");
			while( monThread.isAlive() )
			{
				if ( debug )
					RXTXlog.printDebug( "	MonThread is still alive!");
				try {
					monThread.join(1000);
					Thread.sleep( 1000 );
				} catch( Exception e ){} 
		...
		if (debug)
			RXTXlog.printDebug( "	RXTXPort:calling gc()");
		monThread = null;
		SPEventListener = null;
		MonitorThreadLock = false;
		MonitorThreadAlive=false;
		monThreadisInterrupted=true;
		RXTXlog.printDebug( "RXTXPort:removeEventListener() returning");
	}
	/**
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:notifyOnDataAvailable( " +
				enable+" )");
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:notifyOnOutputEmpty( " +
				enable+" )");
		waitForTheNativeCodeSilly();
		MonitorThreadLock = true;
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:notifyOnCTS( " +
				enable+" )");
		waitForTheNativeCodeSilly();
		MonitorThreadLock = true;
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:notifyOnDSR( " +
				enable+" )");
		waitForTheNativeCodeSilly();
		MonitorThreadLock = true;
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:notifyOnRingIndicator( " +
				enable+" )");
		waitForTheNativeCodeSilly();
		MonitorThreadLock = true;
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:notifyOnCarrierDetect( " +
				enable+" )");
		waitForTheNativeCodeSilly();
		MonitorThreadLock = true;
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:notifyOnOverrunError( " +
				enable+" )");
		waitForTheNativeCodeSilly();
		MonitorThreadLock = true;
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:notifyOnParityError( " +
				enable+" )");
		waitForTheNativeCodeSilly();
		MonitorThreadLock = true;
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:notifyOnFramingError( " +
				enable+" )");
		waitForTheNativeCodeSilly();
		MonitorThreadLock = true;
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:notifyOnBreakInterrupt( " +
				enable+" )");
		waitForTheNativeCodeSilly();
		MonitorThreadLock = true;
	...
	*/
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:close( " + this.name + " )"); 
		if( closeLock ) return;
		closeLock = true;
		while( IOLocked > 0 )
		{
			if( debug )
				RXTXlog.printDebug("IO is locked " + IOLocked);
			try {
				Thread.sleep(500);
			} catch( Exception e ) {}
		}
		if ( fd <= 0 )
		{
			RXTXlog.printDebug(  "RXTXPort:close detected bad File Descriptor" );
			return;
		}
		setDTR(false);
		setDSR(false);
		if (debug)
			RXTXlog.printDebug( "RXTXPort:close( " + this.name + " ) setting monThreadisInterrupted"); 
		if ( ! monThreadisInterrupted )
		{
			removeEventListener();
		}
		if (debug)
			RXTXlog.printDebug( "RXTXPort:close( " + this.name + " ) calling nativeClose"); 
		nativeClose( this.name );
		if (debug)
			RXTXlog.printDebug( "RXTXPort:close( " + this.name + " ) calling super.close"); 
		super.close();
		fd = 0;
		closeLock = false;
		if (debug)
			RXTXlog.printDebug( "RXTXPort:close( " + this.name + " ) leaving"); 
	}
	...
	{
		if (debug)
			RXTXlog.printDebug( "RXTXPort:finalize()");
		if( fd > 0 )
		{
			if (debug)
				RXTXlog.printDebug( "RXTXPort:calling close()");
			close();
		}
	}
	...
	*/
		...
		{
			if (debug_write)
				RXTXlog.printDebug( "RXTXPort:SerialOutputStream:write(int)");
			if( speed == 0 ) return;
			if ( monThreadisInterrupted == true )
			{
				return;
			...
			try
			{
				...
				if (debug_write)
					RXTXlog.printDebug( "Leaving RXTXPort:SerialOutputStream:write( int )");
			}
			catch( IOException e )
			{
				IOLocked--;
	...
	*/
		public void write( byte b[] ) throws IOException
		{
			if (debug_write)
			{
				RXTXlog.printDebug( "Entering RXTXPort:SerialOutputStream:write(" + b.length + ") "/* + new String(b)*/ );
			}
			if( speed == 0 ) return;
			if ( monThreadisInterrupted == true )
			{
			...
			try
			{
				...
				if (debug_write)
					RXTXlog.printDebug( "Leaving RXTXPort:SerialOutputStream:write(" +b.length  +")");
			}
			catch( IOException e )
			{
				IOLocked--;
	...
	*/
			...
			throws IOException
		{
			...
			if (debug_write)
			{
				RXTXlog.printDebug( "Entering RXTXPort:SerialOutputStream:write(" + send.length + " " + off + " " + len + " " +") " /*+  new String(send) */ );
			}
			if ( fd == 0 ) throw new IOException();
			if ( monThreadisInterrupted == true )
			{
			...
			try
			{
				...
				if( debug_write )
					RXTXlog.printDebug( "Leaving RXTXPort:SerialOutputStream:write(" + send.length + " " + off + " " + len + " " +") "  /*+ new String(send)*/ );
			}
			catch( IOException e )
			{
				IOLocked--;
	...
	*/
		...
		{
			if (debug)
				RXTXlog.printDebug( "RXTXPort:SerialOutputStream:flush() enter");
			if( speed == 0 ) return;
			if ( fd == 0 ) throw new IOException();
			if ( monThreadisInterrupted == true )
			{
			if (debug)
				RXTXlog.printDebug( "RXTXPort:SerialOutputStream:flush() Leaving Interrupted");
				return;
			}
			IOLocked++;
			waitForTheNativeCodeSilly();
			...
			*/
			try
			{
				...
				if (debug)
					RXTXlog.printDebug( "RXTXPort:SerialOutputStream:flush() leave");
			}
			catch( IOException e )
			{
				IOLocked--;
...
*                play a role
	*/
		...
		{
			if (debug_read)
				RXTXlog.printDebug( "RXTXPort:SerialInputStream:read() called");
			if ( fd == 0 ) throw new IOException();
			if ( monThreadisInterrupted )
			{
				RXTXlog.printDebug( "+++++++++ read() monThreadisInterrupted" );
			}
			IOLocked++;
			if (debug_read_results)
				RXTXlog.printDebug(  "RXTXPort:SerialInputStream:read() L" );
			waitForTheNativeCodeSilly();
			if (debug_read_results)
				RXTXlog.printDebug(  "RXTXPort:SerialInputStream:read() N" );
			try
			{
				int result = readByte();
				if (debug_read_results)
					//RXTXlog.printDebug(  "RXTXPort:SerialInputStream:read() returns byte = " + result );
					RXTXlog.printDebug(  "RXTXPort:SerialInputStream:read() returns" );
				return( result );
			}				
			finally
			{
...
*0       >0      blocks until reads threshold bytes
	*/
		public synchronized int read( byte b[] ) throws IOException
		{
			int result;
			if (debug_read)
				RXTXlog.printDebug( "RXTXPort:SerialInputStream:read(" + b.length + ") called");
			if ( monThreadisInterrupted == true )
			{
				return(0);
			}
			...
			try
			{
				...
				if (debug_read_results)
					RXTXlog.printDebug(  "RXTXPort:SerialInputStream:read() returned " + result + " bytes" );
				return( result );
			}
			finally
			{
...
*0       >0      blocks until either threshold # of bytes or len bytes,
	...
	*/
		...
		{
			if (debug_read)
				RXTXlog.printDebug( "RXTXPort:SerialInputStream:read(" + b.length + " " + off + " " + len + ") called" /*+ new String(b) */ );
			int result;
			/*
			 * Some sanity checks
			 */
			...
			{
				if (debug_read)
					RXTXlog.printDebug( "RXTXPort:SerialInputStream:read() fd == 0");
				RXTXlog.printDebug("+++++++ IOException()\n");
				throw new IOException();
			}

			if( b==null )
			{
				RXTXlog.printDebug("+++++++ NullPointerException()\n");
				if (debug_read)
					RXTXlog.printDebug( "RXTXPort:SerialInputStream:read() b == 0");
				throw new NullPointerException();
			}

			if( (off < 0) || (len < 0) || (off+len > b.length))
			{
				RXTXlog.printDebug("+++++++ IndexOutOfBoundsException()\n");
				if (debug_read)
					RXTXlog.printDebug( "RXTXPort:SerialInputStream:read() off < 0 ..");
				throw new IndexOutOfBoundsException();
			}
			...
			{
				if (debug_read)
					RXTXlog.printDebug( "RXTXPort:SerialInputStream:read() Interrupted");
				return(0);
			}
			IOLocked++;
			waitForTheNativeCodeSilly();
			try
			{
				...
				if (debug_read_results)
					RXTXlog.printDebug( "RXTXPort:SerialInputStream:read(" + b.length + " " + off + " " + len + ") returned " + result + " bytes"  /*+ new String(b) */);
				return( result );
			}
			finally
			{
	...
	*/
		public synchronized int available() throws IOException
		{
			...
			if ( debug_verbose )
				RXTXlog.printDebug( "RXTXPort:available() called" );
			IOLocked++;
			try
			{
				int r = nativeavailable();
				if ( debug_verbose )
					RXTXlog.printDebug( "RXTXPort:available() returning " +
						r );
				return r;
			}
			finally
	...
	*/
	...
	/** Note: these have to be separate boolean flags because the
	   ...
	   defined as integers from 1 to 10  -DPL */
		...
		{
			if (debug)
				RXTXlog.printDebug( "RXTXPort:MontitorThread:MonitorThread()"); 
		}
	/**
	...
	*/
		...
		{
			if (debug)
				RXTXlog.printDebug( "RXTXPort:MontitorThread:run()"); 
			monThreadisInterrupted=false;
			eventLoop();
			if (debug)
				RXTXlog.printDebug( "eventLoop() returned"); 
		}
		protected void finalize() throws Throwable 
		{ 
			if (debug)
				RXTXlog.printDebug( "RXTXPort:MonitorThread exiting"); 
		}
	}
	/**
...
/*------------------------  END OF CommAPI -----------------------------*/
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "RXTXPort:staticSetDSR( " + port +
						" " + flag );
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "RXTXPort:staticSetDTR( " + port +
						" " + flag );
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "RXTXPort:staticSetRTS( " + port +
						" " + flag );
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "RXTXPort:staticIsRTS( " + port + " )" );
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug( "RXTXPort:staticIsCD( " + port + " )" );
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "RXTXPort:staticIsCTS( " + port + " )" );
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "RXTXPort:staticIsDSR( " + port + " )" );
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "RXTXPort:staticIsDTR( " + port + " )" );
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "RXTXPort:staticIsRI( " + port + " )" );
	...
	*/
		...
		throws UnsupportedCommOperationException
	{
		byte ret;
		if ( debug )
			RXTXlog.printDebug(  "getParityErrorChar()" );
		ret = nativeGetParityErrorChar();
		if ( debug )
			RXTXlog.printDebug(  "getParityErrorChar() returns " +
						ret );
		return( ret );
	}
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "setParityErrorChar(" + b + ")" );
	...
	*/
		...
		throws UnsupportedCommOperationException
	{
		byte ret;
		if ( debug )
			RXTXlog.printDebug(  "getEndOfInputChar()" );
		ret = nativeGetEndOfInputChar();
		if ( debug )
			RXTXlog.printDebug(  "getEndOfInputChar() returns " +
						ret );
		return( ret );
	}
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "setEndOfInputChar(" + b + ")" );
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "RXTXPort:setUARTType()");
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "RXTXPort:setBaudBase()");
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "RXTXPort:getBaudBase()");
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "RXTXPort:setDivisor()");
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "RXTXPort:getDivisor()");
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "RXTXPort:setLowLatency()");
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "RXTXPort:getLowLatency()");
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "RXTXPort:setCallOutHangup()");
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "RXTXPort:getCallOutHangup()");
	...
	*/
	...
	{
		if ( debug )
			RXTXlog.printDebug(  "RXTXPort:clearCommInput()");
