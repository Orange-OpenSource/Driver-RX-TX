...
//	private final static boolean debug = false;
	...
	static
	{
		RXTXlog.printDebug("RXTXCommDriver {}");
		...
		*/
		...
		RXTXlog.printInfo("Stable Library");
		RXTXlog.printInfo("=========================================");
		RXTXlog.printInfo("Native lib Version = " + LibVersion);
		RXTXlog.printInfo("Java lib Version   = " + JarVersion );

		if ( ! JarVersion.equals( LibVersion ) )
		{
			RXTXlog.printWarning("WARNING:  RXTX Version mismatch\n\tJar version = " + JarVersion + "\n\tnative lib Version = " + LibVersion);
		}
		RXTXlog.printDebug("RXTXCommDriver:\n\tJar version = " + JarVersion + "\n\tnative lib Version = " + LibVersion );
	}
...
//		RXTXlog.printDebug("RXTXCommDriver:getValidPortPrefixes()");
//		if(CandidatePortPrefixes==null)
//		{
//			RXTXlog.printDebug("RXTXCommDriver:getValidPortPrefixes() No ports prefixes known for this System.\nPlease check the port prefixes listed for " + osName + " in RXTXCommDriver:registerScannedPorts()");
//		}
//		int i=0;
...
////				}
////			}
//			RXTXlog.printDebug("RXTXCommDriver:getValidPortPrefixes() No ports matched the list assumed for this System in the directory " + deviceDirectory);
//		}
//		else
//		{
//			RXTXlog.printDebug("RXTXCommDriver:getValidPortPrefixes(),The following port prefixes have been identified as valid on " + osName);
//
//			for(int j=0;j<returnArray.length;j++)
//			{
//				RXTXlog.printDebug("\t" + j + " " +
//						returnArray[j]);
//			}
//
//			
...
//		}
		...
		RXTXlog.printDebug("Leaving registerValidPorts()");
	}
	...
	*/
	public void initialize()
	{
		...
		RXTXlog.printDebug("RXTXCommDriver:initialize()");
...
//		}
	...
	private void addSpecifiedPorts(String names, int PortType)
	{
		...
		RXTXlog.printDebug("RXTXCommDriver:addSpecifiedPorts()");
		...
		try
		    {
		    	...
		    	RXTXlog.printDebug("The file: gnu.io.rxtx.properties doesn't exists.");
		    	RXTXlog.printDebug(e.toString());
			}//end catch

		    RXTXlog.printDebug("checking for system-known ports of type "+PortType);
		    RXTXlog.printDebug("checking registry for ports of type "+PortType);
...
//				break;
			default:
				RXTXlog.printDebug("unknown port type "+PortType+" passed to RXTXCommDriver.registerSpecifiedPorts()");
		}
    ...
    */
	private void registerScannedPorts(int PortType)
	{
		...
		RXTXlog.printDebug("scanning device directory "+deviceDirectory+" for ports of type "+PortType);
	...
	*/
			...
			{
				RXTXlog.printDebug("RXTXCommDriver:registerScannedPorts() no Device files to check ");
				return;
			}
			...
			switch (PortType) {
				case CommPortIdentifier.PORT_SERIAL:
					RXTXlog.printDebug("scanning for serial ports for os "+osName);
			...
			*/
					...
					{
						String[] Temp = {
						"ttyACM", // linux modem drivers
						"ttyS", // linux Serial Ports
						"ttySA", // for the IPAQs
						"ttyUSB", // for USB frobs
						"ttyHS" // for HS drivers
						};
				...
				{
					RXTXlog.printDebug("No valid prefixes for serial ports have been entered for "+osName + " in RXTXCommDriver.java.  This may just be a typo in the method registerScanPorts().");
				}
				break;
...
//				break;
			default:
				RXTXlog.printDebug("Unknown PortType "+PortType+" passed to RXTXCommDriver.registerScannedPorts()");
		}
	...
	*/
	...
	{
		CommPort p = null;
		RXTXlog.printDebug("RXTXCommDriver:getCommPort("+PortName+","+PortType+")");
		try {
			switch (PortType) {
				case CommPortIdentifier.PORT_SERIAL:
					if(osName.toLowerCase().indexOf("windows") == -1 )
					{
						p = new RXTXPort( PortName );
						return p;
					}
					else
					{
						p = new RXTXPort( deviceDirectory + PortName );
						return p;
					}
//				case CommPortIdentifier.PORT_PARALLEL:
//					return new LPRPort( PortName );
				default:
					RXTXlog.printDebug("unknown PortType  "+PortType+" passed to RXTXCommDriver.getCommPort()");
			}
		} catch( PortInUseException e ) {
			RXTXlog.printError("RXTXCommDriver.getCommPort catch exception: Port "+PortName+" in use by another application");
			return null;
		} catch( NoSuchPortException e ) {			
			RXTXlog.printError("RXTXCommDriver.getCommPort catch exception " + e.toString());
			return null;
		} 
		return null;
	}
