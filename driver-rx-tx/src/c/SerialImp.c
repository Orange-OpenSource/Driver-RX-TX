...
void print_log(JNIEnv *env, int level, char* message)
{
	jstring jmess = (*env)->NewStringUTF(env, message);
	jclass jclazz = (*env)->FindClass(env, "gnu/io/RXTXlog");
	jmethodID mid;
	if(jclazz == NULL){
//		fprintf(stderr,"class RXTXlog not found, print message console:");
...
//		fprintf(stderr, "\n");
	...
	switch (level){
		case LOG_ERROR:
			mid = (*env)->GetStaticMethodID(env, jclazz, "printError", "(Ljava/lang/String;)V");
			if(mid != NULL){
					(*env)->CallStaticVoidMethod(env, jclazz, mid, jmess);
					return;
			}
			else{
				fprintf(stderr, "printError, no such method, print to console:");
				fprintf(stderr,message);
				fprintf(stderr, "\n");
				return;
			}
			return;
		case LOG_WARNING:
			mid = (*env)->GetStaticMethodID(env, jclazz, "printWarning", "(Ljava/lang/String;)V");
			if(mid != NULL){
					(*env)->CallStaticVoidMethod(env, jclazz, mid, jmess);
					return;
			}
			else{
				fprintf(stderr, "printWarning, no such method, print to console:");
				fprintf(stderr,message);
				fprintf(stderr, "\n");
				return;
			}
			return;
		case LOG_INFO:
			mid = (*env)->GetStaticMethodID(env, jclazz, "printInfo", "(Ljava/lang/String;)V");
			if(mid != NULL){
					(*env)->CallStaticVoidMethod(env, jclazz, mid, jmess);
					return;
			}
			else{
				fprintf(stdout, "printInfo, no such method, print to console:");
			...
			return;
		case LOG_DEBUG:
			mid = (*env)->GetStaticMethodID(env, jclazz, "printDebug", "(Ljava/lang/String;)V");
			if(mid != NULL){
					(*env)->CallStaticVoidMethod(env, jclazz, mid, jmess);
					return;
			}
			else{
				fprintf(stdout, "printDebug, no such method, print to console:");
				fprintf(stdout, message);
				fprintf(stdout, "\n");
				return;
			}
			return;
		default:
			fprintf(stdout, "default, print to console:");
			fprintf(stdout, message);
			return;
	}
}
	...
	)
{
...
#endif /* !WIN32 */
	print_log(env, LOG_DEBUG, "entring RXTXPort:Initialize");
#ifdef PRERELEASE
	...
	*/
	print_log(env, LOG_DEBUG, "leaving RXTXPort:Initialize");
#endif /* DEBUG && __linux__ */
}
...
void set_java_vars( JNIEnv *env, jobject jobj, int fd )
{
	...
	{
		print_log(env, LOG_DEBUG, "Cannot Get Serial Port Settings");
		(*env)->DeleteLocalRef( env, jclazz );
		return;
	}
	...
	)
{
	...
	(*env)->DeleteLocalRef( env, jclazz );
 	 ...
 	 * ADDED by OU
 	 *
 	 * test the existence of the device file (ex: /dev/ttyUSBx)
 	 * if no such file, that's to say device has leaved
 	 * return -2
	 * java side will throw now such port exception when it receive -2
 	*/

 	if(stat(filename, &st)!= 0)
 	{
 		(*env)->ReleaseStringUTFChars( env, jstr, filename );
// 		throw_java_exception( env, NO_SUCH_PORT_EXCEPTION, "thrown from the native method: open",
// 				strerror( errno ) );
 		return -2;
 	}
	/*
	...
	*/
 	...
 	print_log(env, LOG_DEBUG, "entering RXTXPort:open");
	if ( LOCK(env, filename, pid ) )
	{
		sprintf( message, "open: locking has failed for %s\n",
			filename );
		print_log(env, LOG_ERROR, message);
		goto fail;
	}
	else
	{
		sprintf( message, "open: locking worked for %s\n", filename );
		print_log(env, LOG_DEBUG, message);
	}
	...
	print_log(env, LOG_DEBUG, message);
	print_log(env, LOG_DEBUG, "leaving RXTXPort:open");
	report_time_end( );
	return (jint)fd;
	...
	jobject jobj,jstring jstr )
{
	...
	print_log(env, LOG_DEBUG, ">nativeClose pid");
	/*
	usleep(10000);
	*/
	if( !pid ) {
		...
		(*env)->ExceptionClear( env );
		(*env)->DeleteLocalRef( env, jclazz );
		print_log(env, LOG_DEBUG, "nativeClose: Close not detecting thread pid");
		return;
	}
	print_log(env, LOG_DEBUG, "<nativeClose: pid");
	...
	*/
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:nativeClose");
	if (fd > 0)
	{
		do {
			result=CLOSE (fd);
		}  while ( result < 0 && errno == EINTR );
		UNLOCK(env, filename, pid );
	}
	print_log(env, LOG_DEBUG, "nativeClose: Delete jclazz");
	(*env)->DeleteLocalRef( env, jclazz );
	print_log(env, LOG_DEBUG, "nativeClose: release filename");
	(*env)->ReleaseStringUTFChars( env, jstr, filename );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:nativeClose");
	report_time_end( );
	return;
}
			...
			int stopBits, int parity )
{
	...
	if( tcgetattr( fd, &ttyset ) < 0 )
	{
		print_log(env, LOG_DEBUG, "set_port_params: Cannot Get Serial Port Settings");
		return(1);
	}

	if( translate_data_bits( env, &(ttyset.c_cflag), dataBits ) )
	{
		print_log(env, LOG_DEBUG, "set_port_params: Invalid Data Bits Selected");
		return(1);
	}

	if( translate_stop_bits( env, &(ttyset.c_cflag), stopBits ) )
	{
		print_log(env, LOG_DEBUG, "set_port_params: Invalid Stop Bits Selected");
		return(1);
	}

	if( translate_parity( env, &(ttyset.c_cflag), parity ) )
	{
		print_log(env, LOG_DEBUG, "set_port_params: Invalid Parity Selected");
		return(1);
	}
	...
	{
		print_log(env, LOG_DEBUG, "set_port_params: Cannot Set Speed");
		return( 1 );
	}
#endif  /* __FreeBSD__ */
	if( !cspeed )
		...
		cfsetospeed( &ttyset, cspeed ) < 0 )
	{
		...
		*/
			...
			cfsetospeed( &ttyset, cspeed ) < 0 )
		{
			...
			print_log(env, LOG_DEBUG, "nativeSetSerialPortParams: Cannot Set Speed");
			return( 1 );
		}
	...
	{
		print_log(env, LOG_DEBUG, "tcsetattr returns nonzero value!");
		return( 1 );
	}
	return(0);
}
	...
	jint parity )
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:nativeSetSerialPortParams");
	report_time_start( );

	if (cspeed < 0 )
	{
		print_log(env, LOG_DEBUG, "invalid cspeed");
/*
...
*/
	...
	{
		print_log(env, LOG_DEBUG, "set_port_params failed");
		print_log(env, LOG_DEBUG, "leaving RXTXPort:nativeSetSerialPortParams");
/*
    For some reason the native exceptions are not being caught.  Moving this
    to the Java side fixed the issue.  taj.
		throw_java_exception( env, UNSUPPORTED_COMM_OPERATION,
...
{
	print_log(env, LOG_DEBUG, "entering RXTXPort:translate_speed");
	switch( speed ) {
		case 0:		return B0;
		case 50:	return B50;
		case 75:	return B75;
...
/* I don't think this is universal.. older UARTs never did these.  taj */
	...
	{
		print_log(env, LOG_DEBUG, "leaving RXTXPort:translate_speed: Error condition");
		return -1;
	}
}
...
int translate_data_bits( JNIEnv *env, tcflag_t *cflag, jint dataBits )
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:translate_date_bits");
	switch( dataBits ) {
		case JDATABITS_5:
			(*cflag) = temp | CS5;
			return 0;
	...
	print_log(env, LOG_DEBUG, "leaving RXTXPort:translate_date_bits");
/*
    For some reason the native exceptions are not being caught.  Moving this
    to the Java side fixed the issue.  taj.
	throw_java_exception( env, UNSUPPORTED_COMM_OPERATION,
...
{
	print_log(env, LOG_DEBUG, "entering RXTXPort:translate_stop_bits");
	switch( stopBits ) {
		case STOPBITS_1:
			(*cflag) &= ~CSTOPB;
			print_log(env, LOG_DEBUG, "leaving RXTXPort:translate_stop_bits");
			return 0;
...
JNIEXPORT jint JNICALL RXTXPort(nativeGetParity)(JNIEnv *env, jobject jobj, jint fd)
{
	...
	if( tcgetattr( fd, &ttyset ) < 0 )
	{
		print_log(env, LOG_DEBUG, "nativeGetParity:  tcgetattr failed");
		return( -1 );
	}
#ifdef  CMSPAR
...
{
	char message[80];
	print_log(env, LOG_DEBUG, "entering translate_parity");
#ifdef CMSPAR
	...
	switch( parity ) {
		case JPARITY_NONE:
			...
			print_log(env, LOG_DEBUG, "leaving translate_parity");
			return 0;
		case JPARITY_EVEN:
			(*cflag) |= PARENB;
...
#endif /* CMSPAR */
		default:
			sprintf(message, "Parity missed %i\n", (int) parity);
			print_log(env, LOG_DEBUG, message);
	}
...
void *drain_loop( void *arg )
{
	...
	for(i=0;;i++)
	{
		...
		//print_log(env, LOG_DEBUG, "drain_loop:  looping\n");
#if defined(__sun__)
		...
		*/
		...
		if( tcdrain( eis->fd ) == 0 )
		{
			...
			else
			{
				...
				//print_log(env, LOG_DEBUG, "drain_loop:  writing not set\n");
			}
		}
		else
		{
			//report("drain_loop:  tcdrain bad fd\n");
			//print_log(env, LOG_DEBUG, "drain_loop:  tcdrain bad fd\n");
			goto end;
		}
	}
end:
	//report("------------------ drain_loop exiting ---------------------\n");
	//print_log(env, LOG_DEBUG, "------------------ drain_loop exiting ---------------------");
	eis->closing = 1;
	pthread_exit( NULL );
	return( NULL );
}
...
void finalize_threads( struct event_info_struct *eis )
{
	...
	//print_log(env, LOG_DEBUG,"entering finalize_threads");

	/* need to clean up again after working events */
	//LEAVE("---------------- finalize_threads ---------------");
	//print_log(env, LOG_DEBUG,"leaving---------------- finalize_threads ---------------");
	...
	jobject jobj, jint ji, jboolean interrupted )
{
	...
	report_time_start();
	print_log(env, LOG_DEBUG, "entering RXTXPort:writeByte");
	do {
		sprintf( msg, "writeByte %c>>\n", byte );
		print_log(env, LOG_DEBUG, msg);
...
/*
	...
	if( ! interrupted )
	{
		...
		index->writing = 1;
		print_log(env, LOG_DEBUG, "writeByte:  index->writing = 1");
	}
#endif
	sprintf( msg, "RXTXPort:writeByte %i\n", result );
	print_log(env, LOG_DEBUG, msg);
	print_log(env, LOG_DEBUG, "leaving RXTXPort:writeByte");
	if(result >= 0)
	{
		report_time_end();
		return;
...
/* return; OH CRAP */
	...
	report_time_start();
	print_log(env, LOG_DEBUG, "entering writeArray");
	/* warning Roy Rogers */
	/*
	...
	*/
	...
	do {
		...
		print_log(env, LOG_DEBUG, "writeArray()");
...
/*
	...
	if( !interrupted )
	{
		...
		index->writing = 1;
		print_log(env, LOG_DEBUG, "writeArray:  index->writing = 1");
	}
#endif /* TIOCSERGETLSR */
	/*
	...
	*/
	print_log(env, LOG_DEBUG,"leaving RXTXPort:writeArray" );
	report_time_end();
fail:
	...
	jobject jobj, jboolean interrupted )
{
	...
	print_log(env, LOG_DEBUG,"entering SerialImp.c:drain()");
	report_time_start( );

	/* on linux, if the device leaved already, execute tcdrain() might dump the kernel */
//	do {
//		print_log(env, LOG_DEBUG,"nativeDrain: trying tcdrain");
//		result=tcdrain(fd);
//		count++;
//	}  while (result && errno==EINTR && count <3);
//
...
//	print_log(env, LOG_DEBUG, message);
	...
	return( JNI_TRUE );
#endif /* __sun__ */
	print_log(env, LOG_DEBUG, "leaving RXTXPort:drain()");
	...
	jobject jobj, jint duration )
{
	int fd = get_java_var( env, jobj,"fd","I" );
	report_time_start( );
	print_log(env, LOG_DEBUG,"entering RXTXPort:sendBreak()");
	tcsendbreak( fd, (int)( duration / 250 ) );
	report_time_end( );
	print_log(env, LOG_DEBUG,"leaving RXTXPort:sendBreak()");
	...
	)
{
	...
	print_log(env, LOG_DEBUG,"entering RXTXPort:nativegetRecieveTimeout()");
	if( tcgetattr( fd, &ttyset ) < 0 ) goto fail;
	print_log(env, LOG_DEBUG,"leaving RXTXPort:nativegetRecieveTimeout()");
	...
	)
{
	int fd = get_java_var( env, jobj,"fd","I" );
	struct termios ttyset;
	print_log(env, LOG_DEBUG, "RXTXPort:NativeisRecieveTimeoutEnabled()" );
	if( tcgetattr( fd, &ttyset ) < 0 ) goto fail;
	print_log(env, LOG_DEBUG, "leaving RXTXPort:NativeisRecieveTimeoutEnabled()" );
	...
	jobject jobj )
{
	...
	print_log(env, LOG_DEBUG, "RXTXPort:isDSR" );
	ioctl( fd, TIOCMGET, &result );
	sprintf( message, "RXTXPort:isDSR returns %i\n", result & TIOCM_DSR );
	print_log(env, LOG_DEBUG, message );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:isDSR" );
	...
	jobject jobj )
{
	...
	print_log(env, LOG_DEBUG, "RXTXPort:isCD" );
	ioctl( fd, TIOCMGET, &result );
	sprintf( message, "RXTXPort:isCD returns %i\n", result & TIOCM_CD );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:isCD" );
	...
	jobject jobj )
{
	...
	print_log(env, LOG_DEBUG, "RXTXPort:isCTS" );
	ioctl( fd, TIOCMGET, &result );
	sprintf( message, "RXTXPort:isCTS returns %i\n", result & TIOCM_CTS );
	print_log(env, LOG_DEBUG, message );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:isCTS" );
	...
	jobject jobj )
{
	...
	print_log(env, LOG_DEBUG, "RXTXPort:isRI" );
	ioctl( fd, TIOCMGET, &result );
	sprintf( message, "RXTXPort:isRI returns %i\n", result & TIOCM_RI );
	print_log(env, LOG_DEBUG, message );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:isRI" );
	...
	jobject jobj )
{
	...
	print_log(env, LOG_DEBUG, "RXTXPort:isRTS" );
	ioctl( fd, TIOCMGET, &result );
	sprintf( message, "RXTXPort:isRTS returns %i\n", result & TIOCM_RTS );
	print_log(env, LOG_DEBUG, message );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:isRTS" );
	...
	jobject jobj, jboolean state )
{
	...
	print_log(env, LOG_DEBUG, "RXTXPort:setRTS" );
	ioctl( fd, TIOCMGET, &result );
	...
	print_log(env, LOG_DEBUG, message );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:setRTS" );
	return;
}
	...
	jobject jobj, jboolean state )
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:setDSR()" );
	ioctl( fd, TIOCMGET, &result );
	...
	sprintf( message, "setDSR( %i )\n", state );
	print_log(env, LOG_DEBUG, message );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:setDSR()" );
	return;
}
	...
	jobject jobj )
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:isDTR" );
	ioctl( fd, TIOCMGET, &result );
	sprintf( message, "isDTR( ) returns %i\n", result& TIOCM_DTR );
	print_log(env, LOG_DEBUG, message );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:isDTR" );
	...
	jobject jobj, jboolean state )
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:setDTR" );
	ioctl( fd, TIOCMGET, &result );
	...
	print_log(env, LOG_DEBUG, message );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:setDTR" );
	return;
}
/*----------------------------------------------------------
	...
	jclass jclazz, jstring jstr, jboolean flag)
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:nativeStaticSetDSR" );
#ifndef WIN32
	pid = getpid();
#endif /* WIN32 */
	...
	if ( LOCK(env, filename, pid ) ) goto fail;
	...
	(*env)->ReleaseStringUTFChars( env, jstr, filename );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:nativeStaticSetDSR" );
	return( JNI_TRUE );
fail:
	(*env)->ReleaseStringUTFChars( env, jstr, filename );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:nativeStaticSetDSR" );
	...
	jclass jclazz, jstring jstr, jboolean flag)
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:nativeStaticSetRTS" );
#ifndef WIN32
	pid = getpid();
#endif /* WIN32 */
	...
	(*env)->ReleaseStringUTFChars( env, jstr, filename );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:nativeStaticSetRTS" );
	return( JNI_TRUE );
fail:
	(*env)->ReleaseStringUTFChars( env, jstr, filename );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:nativeStaticSetRTS" );
	...
	jclass jclazz, jstring jstr, jint baudrate, jint dataBits, jint stopBits, jint parity )
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:nativeStaticSetSerialPortParams" );
#ifndef WIN32
	pid = getpid();
#endif /* WIN32 */
	/* Open and lock the port so nothing else changes the setting */
	...
	{
		(*env)->ReleaseStringUTFChars( env, jstr, filename );
		print_log(env, LOG_DEBUG, "leaving RXTXPort:nativeStaticSetSerialPortParams" );
		throw_java_exception( env, UNSUPPORTED_COMM_OPERATION,
	...
	{
		(*env)->ReleaseStringUTFChars( env, jstr, filename );
		print_log(env, LOG_DEBUG, "leaving RXTXPort:nativeStatic SetSerialPortParams" );
		throw_java_exception( env, UNSUPPORTED_COMM_OPERATION,
			"nativeStaticSetSerialPortParams", strerror( errno ) );
		return;
	}
	...
	jclass jclazz, jstring jstr, jboolean flag)
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:nativeStaticSetDTR" );
#ifndef WIN32
	pid = getpid();
#endif /* WIN32 */
	...
	(*env)->ReleaseStringUTFChars( env, jstr, filename );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:nativeStaticSetDTR" );
	return( JNI_TRUE );
fail:
	(*env)->ReleaseStringUTFChars( env, jstr, filename );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:nativeStaticSetDTR" );
	...
	jobject jobj, jstring jstr )
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:nativeStaticIsRTS" );
	if( !fd )
	{
		/* Exception? FIXME */
		return JNI_FALSE;
	...
	print_log(env, LOG_DEBUG, message );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:nativeStaticIsRTS" );
	if( result & TIOCM_RTS ) return JNI_TRUE;
	else return JNI_FALSE;
}
/*----------------------------------------------------------
	...
	jobject jobj, jstring jstr )
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:nativeStaticIsDSR" );
	if( !fd )
	{
		/* Exception? FIXME */
		return JNI_FALSE;
	...
	print_log(env, LOG_DEBUG, message );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:nativeStaticIsDSR" );
	if( result & TIOCM_DSR ) return JNI_TRUE;
	else return JNI_FALSE;
}
/*----------------------------------------------------------
	...
	jobject jobj, jstring jstr )
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:nativeStaticIsDTR" );
	if( !fd )
	{
		/* Exception? FIXME */
		return JNI_FALSE;
	...
	print_log(env, LOG_DEBUG, message );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:nativeStaticIsDTR" );
	if( result & TIOCM_DTR ) return JNI_TRUE;
	else return JNI_FALSE;
}
/*----------------------------------------------------------
	...
	jobject jobj, jstring jstr )
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:nativeStaticIsCD" );
	if( !fd )
	{
		/* Exception? FIXME */
		return JNI_FALSE;
	...
	print_log(env, LOG_DEBUG, message );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:nativeStaticIsCD" );
	if( result & TIOCM_CD ) return JNI_TRUE;
	else return JNI_FALSE;
}
/*----------------------------------------------------------
	...
	jobject jobj, jstring jstr )
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:nativeStaticIsCTS" );
	if( !fd )
	{
		/* Exception? FIXME */
		return JNI_FALSE;
	...
	print_log(env, LOG_DEBUG, message );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:nativeStaticIsCTS" );
	if( result & TIOCM_CTS ) return JNI_TRUE;
	else return JNI_FALSE;
}
/*----------------------------------------------------------
	...
	jobject jobj, jstring jstr )
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:nativeStaticIsRI" );
	if( !fd )
	{
		/* Exception? FIXME */
		return JNI_FALSE;
	...
	print_log(env, LOG_DEBUG, message );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:nativeStaticIsRI" );
	if( result & TIOCM_RI ) return JNI_TRUE;
	else return JNI_FALSE;
}
...
JNIEXPORT jint JNICALL RXTXPort(nativeStaticGetBaudRate)( JNIEnv *env, jobject jobj, jstring jstr )
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:nativeStaticGetBaudRate" );
	if( !fd )
	{
		/* Exception? FIXME */
		return -1;
	...
	if( tcgetattr( fd, &ttyset ) < 0 )
	{
		print_log(env, LOG_DEBUG, "nativeStaticGetBaudRate: Cannot Get Serial Port Settings" );
		return(-1);
	}
/*
dima writes:
...
JNIEXPORT jint JNICALL RXTXPort(nativeStaticGetDataBits)( JNIEnv *env, jobject jobj, jstring jstr )
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:nativeStaticGetDataBits" );
	if( !fd )
	{
		/* Exception? FIXME */
		return -1;
	...
	if( tcgetattr( fd, &ttyset ) < 0 )
	{
		print_log(env, LOG_DEBUG, "nativeStaticGetDataBits: Cannot Get Serial Port Settings" );
		return(-1);
	}
	switch( ttyset.c_cflag&CSIZE ) {
...
JNIEXPORT jint JNICALL RXTXPort(nativeStaticGetParity)( JNIEnv *env, jobject jobj, jstring jstr )
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:nativeStaticGetParity" );
	if( !fd )
	{
		/* Exception? FIXME */
		return -1;
	...
	if( tcgetattr( fd, &ttyset ) < 0 )
	{
		print_log(env, LOG_DEBUG, "nativeStaticGetParity: Cannot Get Serial Port Settings" );
		return(-1);
	}
#ifdef CMSPAR
	switch( ttyset.c_cflag&(PARENB|PARODD|CMSPAR ) ) {
...
JNIEXPORT jint JNICALL RXTXPort(nativeStaticGetStopBits)( JNIEnv *env, jobject jobj, jstring jstr )
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:nativeStaticGetStopBits" );
	if( !fd )
	{
		/* Exception? FIXME */
		return -1;
	...
	if( tcgetattr( fd, &ttyset ) < 0 )
	{
		print_log(env, LOG_DEBUG, "nativeStaticGetStopBits: Cannot Get Serial Port Settings" );
		return(-1);
	}
	switch( ttyset.c_cflag&(CSTOPB) ) {
	...
	jobject jobj )
{
	...
	print_log(env, LOG_DEBUG, "entering nativeGetParityErrorChar" );
#ifdef WIN32
...
#endif /* WIN32 */
	print_log(env, LOG_DEBUG, "leaving nativeGetParityErrorChar" );
	return( ( jbyte ) result );
}
	...
	jobject jobj )
{
	...
	print_log(env, LOG_DEBUG, "entering nativeGetEndOfInputChar" );
	if( tcgetattr( fd, &ttyset ) < 0 ) goto fail;
	print_log(env, LOG_DEBUG, "leaving nativeGetEndOfInputChar" );
	return( (jbyte) ttyset.c_cc[VEOF] );
fail:
	print_log(env, LOG_DEBUG, "leaving nativeGetEndOfInputChar" );
	print_log(env, LOG_DEBUG, "nativeGetEndOfInputChar failed" );
	return( ( jbyte ) -1 );
}
...
#ifdef WIN32
		int fd = get_java_var( env, jobj,"fd","I" );
		print_log(env, LOG_DEBUG, "entering nativeSetParityErrorChar" );
		termiosSetParityError( fd, ( char ) value );
		print_log(env, LOG_DEBUG, "leaving nativeSetParityErrorChar" );
		return( JNI_TRUE );
#else
		print_log(env, LOG_DEBUG, "entering nativeSetParityErrorChar" );
	/*
	...
	jobject jobj, jbyte value )
{
	...
	print_log(env, LOG_DEBUG, "entering nativeSetEndOfInputChar" );
	if( tcgetattr( fd, &ttyset ) < 0 ) goto fail;
	ttyset.c_cc[VEOF] = ( char ) value;
	if( tcsetattr( fd, TCSANOW, &ttyset ) < 0 ) goto fail;
	print_log(env, LOG_DEBUG, "leaving nativeSetEndOfInputChar" );
	return( JNI_TRUE );
fail:
	throw_java_exception( env, IO_EXCEPTION, "nativeSetEndOfInputChar",
		strerror( errno ) );
	print_log(env, LOG_DEBUG, "nativeSetEndOfInputChar failed" );
	print_log(env, LOG_DEBUG, "leaving nativeSetEndOfInputChar" );
	return( JNI_FALSE );
}
...
#endif /* WIN32 */
		if (ret == -1){
			print_log(env, LOG_DEBUG, "read_byte_array: select returned -1" );
			print_log(env, LOG_DEBUG, "leaving read_byte_array" );
			eis->eventflags[SPE_DATA_AVAILABLE] = flag;
			return -1;
		}
		else if (ret > 0)
		{
			if ((ret = READ( fd, buffer + bytes, left )) < 0 ){
				if (errno != EINTR && errno != EAGAIN){
					print_log(env, LOG_DEBUG, "read_byte_array: read returned -1" );
					print_log(env, LOG_DEBUG, "leaving read_byte_array" );
					eis->eventflags[SPE_DATA_AVAILABLE] = flag;
					return -1;
				}
			...
			int timeout )
{
	...
	report_time_start();
	print_log(env, LOG_DEBUG, "entering read_byte_array" );
	sprintf(msg, "read_byte_array requests %i\n", length);
	print_log(env, LOG_DEBUG, msg );
	left = length;
	if (timeout >= 0)
			...
			int timeout )
{
	...
	while( bytes < length )
	{
		...
		*/
		...
		if( ret == 0 )
		{
			print_log(env, LOG_DEBUG, "read_byte_array: select returned 0" );
			print_log(env, LOG_DEBUG, "leaving read_byte_array" );
			break;
		}
		if( ret < 0 )
		...
		if( ret == 0 )
		{
			print_log(env, LOG_DEBUG, "read_byte_array: read returned 0 bytes" );
			print_log(env, LOG_DEBUG, "leaving read_byte_array" );
			break;
		}
		else if( ret < 0 )
	...
	JNIEnv *env, jobject jobj, jint vtime, jint threshold, jint buffer)
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:NativeEnableRecieveTimeoutThreshold" );
	if( tcgetattr( fd, &ttyset ) < 0 ) goto fail;
	...
	print_log(env, LOG_DEBUG, "leaving RXTXPort:NativeEnableRecieveTimeoutThreshold" );
	return;
fail:
	print_log(env, LOG_DEBUG, "leaving RXTXPort:NativeEnableRecieveTimeoutThreshold" );
...
*/
	...
	if( bytes < 0 ) {
		print_log(env, LOG_DEBUG, "leaving RXTXPort:readByte" );
...
*/
	if( (size_t) length > SSIZE_MAX || (size_t) length < 0 ) {
		print_log(env, LOG_DEBUG, "RXTXPort:readArray length > SSIZE_MAX" );
		print_log(env, LOG_DEBUG, "leaving RXTXPort:readArray" );
	...
	(*env)->ReleaseByteArrayElements( env, jbarray, body, 0 );
	if( bytes < 0 ) {
		print_log(env, LOG_DEBUG, "RXTXPort:readArray bytes < 0" );
		print_log(env, LOG_DEBUG, "leaving RXTXPort:readArray" );
...
*/
	...
	do
	{
		...
		{
			print_log(env, LOG_DEBUG, "Got terminator!" );
			break;
		}
...
/*
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:nativeavailable" );
...
*/
	return (jint)result;
fail:
	print_log(env, LOG_DEBUG, "RXTXPort:nativeavailable:  ioctl() failed");
/*
	LEAVE( "RXTXPort:nativeavailable" );
*/
	...
	jobject jobj, jint flowmode )
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:setflowcontrol" );
...
/* TRENT */
	...
	print_log(env, LOG_DEBUG, "leaving RXTXPort:setflowcontrol" );
	return;
fail:
	print_log(env, LOG_DEBUG, "leaving RXTXPort:setflowcontrol" );
	throw_java_exception( env, UNSUPPORTED_COMM_OPERATION, "",
...
int check_line_status_register( struct event_info_struct *eis )
{
	...
	{
		//print_log(env, LOG_DEBUG, "check_line_status_register: fstat\n" );
		return( 1 );
	}
	if( ioctl( eis->fd, TIOCSERGETLSR, &eis->change ) )
	{
		//print_log(env, LOG_DEBUG, "check_line_status_register: TIOCSERGETLSR\n is nonnull\n" );
		return( 1 );
	}
...
#endif /* TIOCSERGETLSR */
	//print_log(env, LOG_DEBUG, "has_line_status_register_acess: Port does not support TIOCSERGETLSR\n" );
	return( 0 );
}
...
void check_cgi_count( struct event_info_struct *eis )
{
	...
	{
		//print_log(env, LOG_DEBUG, "check_cgi_count: TIOCGICOUNT\n is not 0\n" );
		return;
	}
...
void check_tiocmget_changes( struct event_info_struct * eis )
{
	...
	{
		//print_log(env, LOG_DEBUG, "=======================================\n");
		//print_log(env, LOG_DEBUG, "check_tiocmget_changes: ioctl(TIOCMGET)\n" );
		return;
	}
	...
	if( eis && change )
	{
		//print_log(env, LOG_DEBUG, "sending DSR ===========================\n");
...
int initialise_event_info_struct( struct event_info_struct *eis )
{
	...
	if( ioctl( eis->fd, TIOCMGET, &eis->omflags) < 0 ) {
		print_log(env, LOG_DEBUG, "initialise_event_info_struct: Port does not support events" );
	}
	...
	return( 1 );
fail:
fprintf(stderr," debugD");
	print_log(env, LOG_ERROR, "initialise_event_info_struct: initialise failed!");
...
JNIEXPORT void JNICALL RXTXPort(eventLoop)( JNIEnv *env, jobject jobj )
{
	...
	print_log(env, LOG_DEBUG, "entering eventLoop" );
	...
	do{
		...
		do {
			...
			if( eis.closing )
			{
				print_log(env, LOG_DEBUG,"eventLoop: got interrupt\n");
				finalize_threads( &eis );
				finalize_event_info_struct( &eis );
				print_log(env, LOG_DEBUG, "leaving eventLoop" );
				return;
			}
#ifndef WIN32
			/* report( "." ); */
...
*/
	...
	print_log(env, LOG_DEBUG, "leaving eventLoop:  Bailing!" );
	return;
}
...
JNIEXPORT jstring JNICALL RXTXVersion(nativeGetVersion) (JNIEnv *env,
	jclass jclazz )
{
	return (*env)->NewStringUTF( env, "RXTX-2.1-9" );
}
...
)
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXPort:testRead" );
	...
	*/
	...
	if ( LOCK(env, name, pid ) )
	{
		(*env)->ReleaseStringUTFChars(env, tty_name, name);
		print_log(env, LOG_DEBUG, "leaving RXTXPort:testRead no lock" );
		return JNI_FALSE;
	}
	...
	*/
	...
	if ( port_type == PORT_SERIAL )
	{
		...
		{
			print_log(env, LOG_DEBUG, "testRead() fcntl(F_GETFL) failed" );
			ret = JNI_FALSE;
			goto END;
		}
		...
		{
			print_log(env, LOG_DEBUG, "testRead() fcntl(F_SETFL) failed" );
			ret = JNI_FALSE;
			goto END;
		}
		...
		if ( tcsetattr( fd, TCSANOW, &ttyset) < 0 )
		{
			print_log(env, LOG_DEBUG, "testRead() tcsetattr failed" );
			ret = JNI_FALSE;
...
#ifdef EAGAIN
			if ( errno != EAGAIN ) {
				print_log(env, LOG_DEBUG, "testRead() read failed" );
				ret = JNI_FALSE;
			}
#else
#ifdef EWOULDBLOCK
...
END:
	...
	(*env)->ReleaseStringUTFChars( env, tty_name, name );
	CLOSE( fd );
	print_log(env, LOG_DEBUG, "leaving RXTXPort:testRead" );
	return ret;
}
...
registerKnownSerialPorts(JNIEnv *env, jobject jobj, jint portType) /* dima */
{
	...
	jmethodID mid; /* dima */
        ...
        if (cls == 0) { /* dima */
            print_log(env, LOG_DEBUG, "can't find class of gnu/io/CommPortIdentifier" ); /* dima */
	...
	jobject jobj, jstring tty_name)
{
	...
	print_log(env, LOG_DEBUG, "entering RXTXCommDriver:isPortPrefixValid" );
	for(i=0;i<64;i++){
#if defined(__sun__)
...
/* XXX the following hoses freebsd when it tries to open the port later on */
	...
	(*env)->ReleaseStringUTFChars(env, tty_name, name);
	print_log(env, LOG_DEBUG, "leaving RXTXCommDriver:isPortPrefixValid" );
	return(result);
}
...
JNIEXPORT jstring  JNICALL RXTXCommDriver(getDeviceDirectory)(JNIEnv *env,
	jobject jobj)
{
	print_log(env, LOG_DEBUG, "entering RXTXCommDriver:getDeviceDirectory" );
	return (*env)->NewStringUTF(env, DEVICEDIR);
	print_log(env, LOG_DEBUG, "leaving RXTXCommDriver:getDeviceDirectory" );
}
...
{
	print_log(env, LOG_DEBUG, "setInputBufferSize is not implemented" );
}
...
JNIEXPORT jint JNICALL RXTXPort(getInputBufferSize)(JNIEnv *env,
	jobject jobj)
{
	print_log(env, LOG_DEBUG, "getInputBufferSize is not implemented" );
	return(1);
}
...
JNIEXPORT void JNICALL RXTXPort(setOutputBufferSize)(JNIEnv *env,
	jobject jobj, jint size )
{
	print_log(env, LOG_DEBUG, "setOutputBufferSize is not implemented" );
}
...
JNIEXPORT jint JNICALL RXTXPort(getOutputBufferSize)(JNIEnv *env,
	jobject jobj)
{
	print_log(env, LOG_DEBUG, "getOutputBufferSize is not implemented" );
	return(1);
}
...
#endif /* WIN32 */
	print_log(env, LOG_DEBUG,"interruptEventLoop: interrupted\n");
}
...
jboolean is_interrupted( struct event_info_struct *eis )
{
	...
	print_log(env, LOG_DEBUG, "entering is_interrupted" );
	(*env)->ExceptionClear(env);
	result = (*env)->CallBooleanMethod( env, *eis->jobj,
			eis->checkMonitorThread );
#ifdef DEBUG
	if((*env)->ExceptionOccurred(env)) {
		print_log(env, LOG_DEBUG, "is_interrupted: an error occured calling sendEvent()" );
		(*env)->ExceptionDescribe(env);
		(*env)->ExceptionClear(env);
	}
#endif /* DEBUG */
	print_log(env, LOG_DEBUG, "leaving RXTXCommDriver:is_interrupted" );
	return(result);
}
							...
							jboolean flag )
{
	...
	if( !index )
	{
		print_log(env, LOG_ERROR, "nativeSetEventFlag !index");
		return;
	}
	...
	if( index->fd != fd )
	{
		print_log(env, LOG_ERROR, "nativeSetEventFlag !fd");
		return;
	}
...
int send_event( struct event_info_struct *eis, jint type, int flag )
{
	...
	print_log(env, LOG_DEBUG, "entering send_event" );
	if( !eis || eis->eventloop_interrupted > 1 )
	{
		print_log(env, LOG_DEBUG, "event loop interrupted" );
		return JNI_TRUE;
	}
	print_log(env, LOG_DEBUG,"send_event: !eventloop_interupted");
	if(eis->jclazz == NULL) return JNI_TRUE;
	print_log(env, LOG_DEBUG,"send_event: jclazz");

	(*env)->ExceptionClear(env);

	print_log(env, LOG_DEBUG,"send_event: calling");
	result = (*env)->CallBooleanMethod( env, *eis->jobj, eis->send_event,
		type, flag > 0 ? JNI_TRUE : JNI_FALSE );
	print_log(env, LOG_DEBUG,"send_event: called");

#ifdef asdf
	if(!eis || (*eis->env)->ExceptionOccurred(eis->env)) {
		print_log(env, LOG_DEBUG, "send_event: an error occured calling sendEvent()" );
		(*eis->env)->ExceptionDescribe(eis->env);
		(*eis->env)->ExceptionClear(eis->env);
	}
#endif /* asdf */
	/* report("e"); */
	print_log(env, LOG_DEBUG, "leaving send_event" );
	return(result);
}
...
/*
	print_log(env, LOG_DEBUG, "entering get_java_var" );
*/
	if( !jfd ) {
		(*env)->ExceptionDescribe( env );
		(*env)->ExceptionClear( env );
		(*env)->DeleteLocalRef( env, jclazz );
		print_log(env, LOG_DEBUG, "leaving get_java_var" );
		return result;
	}
...
/* ct7 & gel * Added DeleteLocalRef */
	...
	if(!strncmp( "fd",id,2) && result == 0)
		print_log(env, LOG_WARNING, "get_java_var: invalid file descriptor, device file is not opened normally");
/*
	LEAVE( "get_java_var" );
*/
	return result;
...
void throw_java_exception( JNIEnv *env, char *exc, char *foo, char *msg )
{
	...
	print_log(env, LOG_DEBUG, "entering throw_java_exception" );
	if( !clazz ) {
		(*env)->ExceptionDescribe( env );
		(*env)->ExceptionClear( env );
		print_log(env, LOG_DEBUG, "leaving throw_java_exception" );
		return;
	}
#if defined(_GNU_SOURCE)
...
----------------------------------------------------------*/
int lfs_lock(JNIEnv *env, const char *filename, int pid )
{
	int s;
	int ret;
	int size = 1024;
...
----------------------------------------------------------*/
int lfs_unlock(JNIEnv *env, const char *filename, int pid )
{
	int s;
	int ret;
	int size = 1024;
...
----------------------------------------------------------*/
#ifdef LIBLOCKDEV
int lib_lock_dev_lock(JNIEnv *env, const char *filename, int pid )
{
	char message[80];
	printf("LOCKING %s\n", filename);
	if ( dev_testlock( filename ) )
	{
		print_log(env, LOG_DEBUG,  "fhs_lock() lockstatus fail" );
		return 1;
	}
	if ( dev_lock( filename ) )
	{
		sprintf( message,
			"RXTX fhs_lock() Error1: creating lock file for: %s: %s\n",
...
----------------------------------------------------------*/
int fhs_lock(JNIEnv* env, const char *filename, int pid )
{
	/*
	...
	*/
	...
	if ( check_lock_status(env, filename ) )
	{
		print_log(env, LOG_DEBUG, "fhs_lock() lockstatus fail" );
		return 1;
	}
	...
	if( fd < 0 )
	{
		if (errno != EEXIST) {
			sprintf( message,
				"RXTX fhs_lock() Error2: creating lock file: %s: (%d) %s\n",
...
----------------------------------------------------------*/
int uucp_lock(JNIEnv *env, const char *filename, int pid )
{
	...
	if ( check_lock_status(env, filename ) )
	{
		print_log(env, LOG_DEBUG, "RXTX uucp check_lock_status true" );
		return 1;
	}
	if ( stat( LOCKDIR, &buf ) != 0 )
	{
		print_log(env, LOG_DEBUG, "RXTX uucp_lock() could not find lock directory." );
		return 1;
	}
	if ( stat( filename, &buf ) != 0 )
	{
		print_log(env, LOG_DEBUG, "RXTX uucp_lock() could not find device." );
	...
	{
		sprintf( message,
			"RXTX uucp_lock() Error3: creating lock file: %s\n",
			lockfilename );
		print_log(env, LOG_ERROR,  message );
		return 1;
	}
...
/*----------------------------------------------------------
 check_lock_status
   ...
   comments:
----------------------------------------------------------*/
int check_lock_status(JNIEnv *env, const char *filename )
{
	struct stat buf;
	...
	if ( stat( LOCKDIR, &buf ) != 0 )
	{
		print_log(env, LOG_DEBUG, "check_lock_status: could not find lock directory." );
		return 1;
	}

	/*  OK.  Are we able to write to it?  If not lets bail */

	if ( check_group_uucp(env) )
	{
		print_log(env, LOG_ERROR, "check_lock_status: No permission to create lock file.please see: How can I use Lock Files with rxtx? in INSTALL" );
		return(1);
	}

	/* is the device alread locked */
	if ( is_device_locked(env, filename ) )
	{
		print_log(env, LOG_DEBUG, "check_lock_status: device is locked by another application" );
		return 1;
	}
	return 0;
...
/*----------------------------------------------------------
 fhs_unlock
   ...
   comments:    This is for linux only currently.  I see SVR4 does this
		...
		Filesystem Hierachy Standard
----------------------------------------------------------*/
void fhs_unlock(JNIEnv *env, const char *filename, int openpid )
{
	char file[80],*p;
	int i;
	...
	if( !check_lock_pid(env, file, openpid ) )
	{
		unlink(file);
		print_log(env, LOG_DEBUG, "fhs_unlock: Removing LockFile");
	}
	else
	{
		print_log(env, LOG_ERROR, "fhs_unlock: Unable to remove LockFile");
	}
}
...
----------------------------------------------------------*/
void uucp_unlock(JNIEnv *env, const char *filename, int openpid )
{
	struct stat buf;
	...
	if ( stat( filename, &buf ) != 0 )
	{
		...
		print_log(env, LOG_DEBUG, "uucp_unlock() no such device" );
		return;
	}
	...
	{
		/* hmm the file is not there? */
		print_log(env, LOG_DEBUG, "uucp_unlock no such lockfile" );
		return;
	}
	if( !check_lock_pid(env, file, openpid ) )
	{
...
/*----------------------------------------------------------
 check_lock_pid
   ...
   comments:
----------------------------------------------------------*/
int check_lock_pid(JNIEnv *env, const char *file, int openpid )
{
	int fd, lockpid;
	char pid_buffer[12];
	char message[80];
...
----------------------------------------------------------*/
int check_group_uucp(JNIEnv *env)
{
	...
	{
		print_log(env, LOG_ERROR, "check_group_uucp(): Insufficient memory");
		return 1;
	}
	...
	if ( NULL == mktemp(testLockAbsFileName) )
	{
		...
		print_log(env, LOG_ERROR, "check_group_uucp(): mktemp malformed string - \
			should not happen");
	...
	{
		print_log(env, LOG_ERROR, "check_group_uucp(): error testing lock file "
			"creation Error details:");
		print_log(env, LOG_ERROR, strerror(errno));
		free(testLockAbsFileName);
		return 1;
	}
...
int check_group_uucp(JNIEnv *env)
{
	...
	if( user == NULL )
	{
		print_log(env, LOG_ERROR, "Not able to get user groups." );
		return 1;
	} else
	/* JJO changes stop */
	...
	if( user->pw_gid )
	{
		...
		print_log(env, LOG_ERROR, msg );
		print_log(env, LOG_ERROR, UUCP_ERROR );
		return 1;
	}
	return 0;
/*
...
----------------------------------------------------------*/
int is_device_locked(JNIEnv *env, const char *port_filename )
{
			...
			while ( lockprefixes[k] )
			{
				...
				if( stat( file, &buf ) == 0 )
				{
					sprintf( message, UNEXPECTED_LOCK_FILE,
						file );
					print_log(env, LOG_WARNING, message );
					return 1;
				}
	...
	*/
	...
	{
		print_log(env, LOG_DEBUG, "RXTX is_device_locked() could not find device." );
			return 1;
	}
	...
	if( stat( file, &buf ) == 0 )
	{
		...
		snprintf( message, 80, "RXTX info:  is_device_lock return 0 with lock file: %s",file
		);
		print_log(env, LOG_DEBUG, message );
	}
	return 0;
